...
type FilterProps = {
  ...
  selectedOption?: null | FilterOptions;
  onChange: (updatedOptions: string[]) => void;
};

export default function Filter({ options, selectedOption, filterLabel, onChange }: FilterProps) {
  const [isOpen, setIsOpen] = useState(false);
  const currentOption = () => {
    option.map((op: any, index: any) => {
      key = { index };
    });
  };

  const handleCheckboxChange = (value: string) => {
    // const updatedOptions = options.includes(value) ? selectedOptions.filter((item) => item !== value) : [...selectedOptions, value];
