...
// type FilterType = keyof FiltersMetaT['filtersMeta'];
...
// const FilterVariant = ({ label, count }: FilterItemProps) => {
...
// const getTitleFromFilterType = (key: FilterType): string => {
...
// // const renderFilterItems = (key: FilterType, filterData: any[]) => {
// //   switch (key) {
// //     case 'colors':
// //       return filterData.map((item: FilterColor, idx) => <ColorVariant key={item.name || idx} name={item.name} count={item.count || 0} />);
// //     default:
// //       return filterData.map((item: FilterItem, idx: number) => (
// //         <FilterVariant key={idx} label={item.key || item.range} count={item.count || 0} />
// //       ));
// //   }
// // };
...
// const FilterVariantT = ({ label, count }: FilterItemProps) => {
//   const [isChecked, setIsChecked] = useState(false);
//   const setCheckboxChecked = () => setIsChecked(!isChecked);
...
// const FilterGroup = ({ filterItems, className }: FilterGroupType) => {
//   const [isChecked, setIsChecked] = useState(false);
//   const setCheckboxChecked = () => setIsChecked(!isChecked);
...
//           checked={isChecked}
//           onCheckedChange={setCheckboxChecked}
//           className={cn('border border-gray-300 w-5 h-5 rounded-sm opacity-1',
//             isChecked ? 'bg-black text-white' : 'bg-white')}
//         />
//         <p>{filterItems.count}</p>
//     </Container>
//   );
// };
...
// const FilterCategory: React.FC<FilterCategoryProps> = ({ title, items, className }) => {
//   return (
//     <Accordion title={title} icon={DropDownIcon} selectedOptions={['hey', 'bye']}>
//       {items.map((item) => (
//         <FilterGroup key={item.value} filterItems={item} className={className} />
//       ))}
//     </Accordion>
//   );
// };
...
//           const title = getTitleFromFilterType(FilterType);

//           return (
//             <Accordion key={FilterType} title={title} icon={DropDownIcon} selectedOptions={['hey', 'bye']}>
//               <Container className="flex items-center gap-2">
//                 {/* {renderFilterItems(FilterType, filterData)} */}
//                 </Container>
//             </Accordion>
//           );
//         })}
